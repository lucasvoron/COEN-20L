...
float __attribute__((weak)) Root1(float a, float b, float c)
    {
    float top = -b + sqrtf(Discriminant(a, b, c)) ;
    float btm = 2.0 * a ;
    return top / btm ;
    }

float __attribute__((weak)) Root2(float a, float b, float c)
    {
    float top = -b - sqrtf(Discriminant(a, b, c)) ;
    float btm = 2.0 * a ;
    return top / btm ;
...
int main(void)
    ...
    InitializeHardware(HEADER, "Lab 9A: Hardware Fl-Pt Quadratics") ;
...
static BOOL DiscOK(float theirs, float mine)
    ...
    return fabsf(theirs - mine) <= ACCURACY ;
    }

...
uint64_t __attribute__((weak)) TireDiam(uint32_t W, uint32_t A, uint32_t R)
    ...
    ((uint32_t *) &diameter)[1] = R + (A*W)/1270 ; //R1
    ((uint32_t *) &diameter)[0] = (A*W) % 1270 ; //R0
...
uint64_t __attribute__((weak)) TireCirc(uint32_t W, uint32_t A, uint32_t R)
    ...
    ((uint32_t *) &circumference)[1] = numerator / 1587500 ; //R1
    ((uint32_t *) &circumference)[0] = numerator % 1587500 ; //R0
